...
{
    [TestFixture]
    class DOWS_CreateNewCourse_Test : TestBase
    {
        ...
        {
            DOWS_Pagess page = new DOWS_Pagess();
            page.Function_Change();
        ...
        [Test, Order(5)]
        public void NewCourse_Ading()
        {
            DOWS_Pagess newCoursePage = new DOWS_Pagess();
            newCoursePage.New_Course_Button();
            test = extent.StartTest("Click Curriculum tab");
            test.Log(LogStatus.Info, "Click Curriculum Tab");
            driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(10);
        }
